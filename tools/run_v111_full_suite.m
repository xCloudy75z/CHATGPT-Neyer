...
evidence_folder = fullfile(project_root,'audit','v111');
if ~isfolder(evidence_folder), mkdir(evidence_folder); end
output_path = fullfile(evidence_folder,'full-suite-results.txt');
file_id = fopen(output_path,'w');
assert(file_id >= 0,'Could not create the V1.11 full-suite evidence.');
fprintf(file_id,'Neyer V1.11 and general measurement recorder test suite\n');
...
fclose(file_id);
disp(table(results));
fprintf('\nV1.11 FULL SUITE: %d passed, %d failed, %d incomplete.\n', ...
    sum([results.Passed]),sum([results.Failed]),sum([results.Incomplete]));
