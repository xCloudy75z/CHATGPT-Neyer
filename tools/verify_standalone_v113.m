...
live_script = fullfile(project_root,'delivery','Neyer_Gap_Test_v1_13.mlx');
build_source = fullfile(project_root,'delivery','Neyer_Gap_Test_v1_13.m');
evidence_path = fullfile(project_root,'audit','v113','standalone-clean-start.txt');
...
if ~isfile(live_script) || ~isfile(build_source)
    error('verify_standalone_v113:missingFile', ...
        'The V1.13 Live Script or its tracked build source is missing.');
end
exported_source = fullfile(temporary_folder,'exported_v113.m');
matlab.internal.liveeditor.openAndConvert(live_script,exported_source);
exported_text = fileread(exported_source);
build_text=fileread(build_source);
normalise = @(text) strtrim(strrep(text,sprintf('\r\n'),sprintf('\n')));
exported_functions=regexp(exported_text,'(?ms)^function\s.*\z','match','once');
built_functions=regexp(build_text,'(?ms)^function\s.*\z','match','once');
if ~strcmp(normalise(exported_functions),normalise(built_functions))
    error('verify_standalone_v113:embeddedSourceMismatch', ...
        'The functions embedded in the Live Script differ from the tracked build source.');
end
embedded_count = numel(regexp(exported_text,'(?m)^function\s','match'));

isolated_live_script = fullfile(temporary_folder,'Neyer_Gap_Test_v1_13.mlx');
...
if numel(folder_contents) ~= 1
    error('verify_standalone_v113:notIsolated', ...
        'The clean-start folder must contain only the V1.13 Live Script.');
end
...
if isempty(menu_figure)
    error('verify_standalone_v113:noMenu', ...
...
if numel(demo_button) ~= 1
    error('verify_standalone_v113:noDemo','The example button was not found.');
end
...
if isempty(result_figure)
    error('verify_standalone_v113:noResult', ...
        'The embedded published example did not open its result screen.');
end
...
if ~any(contains(label_text,'5.39')) || ~any(contains(label_text,'1.04'))
    error('verify_standalone_v113:wrongDemoResult', ...
...
fprintf(file_id,'MATLAB %s isolated V1.13 Live Script verification passed.\n', ...
    version('-release'));
fprintf(file_id,'The temporary folder contained only Neyer_Gap_Test_v1_13.mlx.\n');
fprintf(file_id,'Embedded local functions: %d\n',embedded_count);
fprintf(file_id,'Embedded functions match the tracked build source exactly.\n');
