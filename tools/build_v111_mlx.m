...
source = fullfile(project_root, 'delivery', 'Neyer_Gap_Test_v1_11.m');
destination = fullfile(project_root, 'delivery', 'Neyer_Gap_Test_v1_11.mlx');
site_destination = fullfile(project_root, 'site', 'downloads', ...
    'Neyer_Gap_Test_v1_11.mlx');
evidence = fullfile(project_root, 'audit', 'v111', 'v111-mlx-build.txt');

if ~isfile(source)
    error('build_v111_mlx:missingSource', ...
        'The generated v1.11 source is missing: %s', source);
end
...
if ~isfile(destination)
    error('build_v111_mlx:notCreated', ...
        'MATLAB did not create the v1.11 Live Script.');
end
...
fprintf(file_id, 'Neyer_Gap_Test_v1_11.mlx created by MATLAB %s.\n', ...
    version('-release'));
