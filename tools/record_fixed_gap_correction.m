...
interaction_cautious_minimum = zeros(size(gaps_mm));
no_interaction_best = zeros(size(gaps_mm));
no_interaction_cautious_minimum = zeros(size(gaps_mm));
...
for row_number = 1:numel(gaps_mm)
    ...
    interaction_cautious_minimum(row_number) = interaction.bound_probability;
    no_interaction_best(row_number) = no_interaction.probability;
    no_interaction_cautious_minimum(row_number) = no_interaction.bound_probability;
end

evidence = table(gaps_mm, interaction_best, interaction_cautious_minimum, ...
    no_interaction_best, no_interaction_cautious_minimum);
output_path = fullfile(project_root, 'audit', 'overnight', ...
    'corrected-fixed-gap-results.csv');
