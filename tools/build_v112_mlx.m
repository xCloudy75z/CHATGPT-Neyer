...
source = fullfile(project_root, 'delivery', 'Neyer_Gap_Test_v1_12.m');
destination = fullfile(project_root, 'delivery', 'Neyer_Gap_Test_v1_12.mlx');
site_destination = fullfile(project_root, 'site', 'downloads', ...
    'Neyer_Gap_Test_v1_12.mlx');
evidence = fullfile(project_root, 'audit', 'v112', 'v112-mlx-build.txt');

if ~isfile(source)
    error('build_v112_mlx:missingSource', ...
        'The generated v1.12 source is missing: %s', source);
end
...
if ~isfile(destination)
    error('build_v112_mlx:notCreated', ...
        'MATLAB did not create the v1.12 Live Script.');
end
...
fprintf(file_id, 'Neyer_Gap_Test_v1_12.mlx created by MATLAB %s.\n', ...
    version('-release'));
