    ...
    if ~has_overlap(levels, successes)
        ...
        if ~part2_started && width > transition_width + comparison_tol
            ...
            if isfield(cfg,'level_increment') && ~isempty(cfg.level_increment)
                x_next = nearest_useful_reachable(x_next,hi_survive, ...
                    lo_break,cfg.level_increment,cfg.min_level);
            end
...
function x = nearest_useful_reachable(raw,hi_no,lo_yes,increment,min_level)
% Nearest grid values whose opposite outcome would create strict overlap.
lower = (ceil(hi_no/increment)-1)*increment;
upper = (floor(lo_yes/increment)+1)*increment;
if lower < min_level
    x = upper;
